...
public class TimeUtils {
	...
	* @throws
	...
	public static List<Date> getBetweenDayListWithoutD2(Date d1, Date d2) {
		List<Date> result = new ArrayList<Date>();
	...
	public static boolean isBenefitSendDate(Date date){
		// 如果不是周一则放弃
		if(TimeUtils.getWeekInt(date) != 1){
			return false;
		}
		Date sTime = TimeUtils.getDayStart(date);
		Date eTime = TimeUtils.getDayEnd(date);
	...
	public static boolean isTodayRefreshTime(){
		Date date = new Date();
		Date sTime = TimeUtils.getDayStart(date);
		Date eTime = TimeUtils.addMins(sTime, 59);
		if(date.equals(sTime) || (date.after(sTime) && date.before(eTime))){
			return true;
		} else {
			return false;
		}
	...
	public static Date getPk10NearestAwardTime(Date d) {
		Calendar c = Calendar.getInstance();
		c.setTime(d);
		if(c.get(Calendar.HOUR_OF_DAY) >= 23 && c.get(Calendar.MINUTE) >= 50){
			int beforeHour = c.get(Calendar.HOUR_OF_DAY);
			...
			c.set(Calendar.SECOND, 0);
			return c.getTime();
		} else if(c.get(Calendar.HOUR_OF_DAY) < 9 || (c.get(Calendar.HOUR_OF_DAY) == 9 && c.get(Calendar.MINUTE) < 30)){
			c.set(Calendar.HOUR_OF_DAY, 9);
			c.set(Calendar.MINUTE, 30);
			c.set(Calendar.SECOND, 0);
			return c.getTime();
			...
			else if(c.get(Calendar.MINUTE) >= 10 && c.get(Calendar.MINUTE) < 30){
				c.set(Calendar.MINUTE, 30);
				c.set(Calendar.SECOND, 0);
				return c.getTime();
			}
			else if(c.get(Calendar.MINUTE) >= 30 && c.get(Calendar.MINUTE) < 50){
				c.set(Calendar.MINUTE, 50);
				c.set(Calendar.SECOND, 0);
				return c.getTime();
			} else if(c.get(Calendar.MINUTE) >= 50){
	...
	public static Date getSpeed10NearestAwardTime(Date d){
		Calendar c = Calendar.getInstance();
		c.setTime(d);
		if(c.get(Calendar.HOUR_OF_DAY) >= 5 && c.get(Calendar.MINUTE) >= 59 && c.get(Calendar.SECOND) >= 45
				&& c.get(Calendar.HOUR_OF_DAY) <= 6 && c.get(Calendar.MINUTE) <= 59 && c.get(Calendar.SECOND) <= 45){
			c.set(Calendar.HOUR_OF_DAY, 6);
			c.set(Calendar.MINUTE, 59);
			c.set(Calendar.SECOND, 45);
			return c.getTime();
		} else {
			if(c.get(Calendar.SECOND) >= 45){
				c.set(Calendar.SECOND, 0);
				c.add(Calendar.MINUTE, 1);
				return c.getTime();
			} else if(c.get(Calendar.SECOND) >= 30 && c.get(Calendar.SECOND) < 45){
				c.set(Calendar.SECOND, 45);
				return c.getTime();
			} else if(c.get(Calendar.SECOND) >= 15 && c.get(Calendar.SECOND) < 30){
	...
	public static Date getSgftNearestAwardTime(Date d){
		Calendar c = Calendar.getInstance();
		c.setTime(d);
		if(c.get(Calendar.MINUTE) >= 55){
			...
			c.set(Calendar.SECOND, 0);
			return c.getTime();
		} else if(c.get(Calendar.MINUTE) >= 0 && c.get(Calendar.MINUTE) < 5){
	...
	public static Date getCqsscNearestAwardTime(Date d) {
		Calendar c = Calendar.getInstance();
		c.setTime(d);
		if(c.get(Calendar.HOUR_OF_DAY) >= 3 && c.get(Calendar.MINUTE) >= 10 && (c.get(Calendar.HOUR_OF_DAY) < 7 || (c.get(Calendar.HOUR_OF_DAY) == 7 && c.get(Calendar.MINUTE) < 30))){
	...
	public static Date getTodayTime(String time){
        ...
        return StringToDate(formatter.format(new Date()) + " " + time);
	}
	
	public static Date getTomorrowTime(String time){
		SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
		
        return StringToDate(formatter.format(addDays(1)) + " " + time);
	}
	
	public static void main(String[] args) {
		/*Date fromTime = new Date();
		Date toTime = TimeUtils.StringToDate("2018-10-23 00:00:00");
		List<Date> list = TimeUtils.getBetweenDayListWithoutD2(toTime, fromTime);
		for(Date d : list){
			System.out.println(d);
		}*/
//		Date d = TimeUtils.StringToDate("2018-10-21 00:00:00");
		
		/*Date d = TimeUtils.StringToDate("2019-03-15 06:30:00");
		
		for(int i=0; i < 44; i++){
			System.out.println(TimeUtils.getNearestAwardTime(d));
			d = TimeUtils.addMins(d, 20);
		}*/
//		System.out.println(getTodayTime("09:30:00"));
		Date toTime = TimeUtils.StringToDate("2019-04-16 00:00:00");
		System.out.println(TimeUtils.getCqsscNearestAwardTime(toTime));
